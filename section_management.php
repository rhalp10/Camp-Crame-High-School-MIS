...
<head>
	...
	<script language='javascript'>
		function updateSection(gradeId,sectionid)
		{
			var newName=prompt('Enter new Section Name:','');
			if (newName==null)
			{				
				return false;
			}
			...
			if (newName!="")
			{
				var newCapacity=prompt('Enter new Section Capacity:','');
				if (newCapacity==null)
				{
					return false;
				}
				else
				{
					...
					if(ans==true)
					{
						window.location="update_section_exec.php?gradeid="+gradeId+"&&sectionid="+sectionid+"&&sectionname="+newName+"&&capacity="+newCapacity;
					}
					else
					{
						return;
	...
	{
		if (f.elements['secname'].value == "" )
		{
			alert("Please enter section name.");
			f.elements['secname'].focus();
			return false;
		}
		else if (f.elements['seccapacity'].value == "" )
		{
			alert("Please enter section capacity.");
			f.elements['seccapacity'].focus();
			return false;
		}
		else
		{
				...
				<!-- CONTENTS -->
	  			<div class="Contents">				
					<div style="position:relative; left:25px;">  
						...
						<br>
						<form action="add_section_exec.php" method="POST" onSubmit="return checkForm(this); return false;" >
							<table class="curvedEdges" width="620" border="1" cellpadding="0" cellspacing="0" bordercolor="#FF6633">
								<td colspan=4 bgcolor="#FF6633"><div align="center"><strong><font color="#FFFFFF">Add New Section</font></strong></div></td>
								<tr>
									<td>Grade Level:</td>
									<td><select autofocus name="gradeid" id="gradeid"  ><?php require_once("myFunctions.php"); displayGradeLevels(); ?></select></td>
								</tr>
								<tr>
									<td>Section Name</td><td colspan=3><input type="text" placeholder="Enter Section Name here..." name="secname" size="65"></td>
								<tr>
								<tr>
									<td>Capacity</td><td colspan=3><input type="text" placeholder="Enter Section Capacity here..." name="seccapacity" size="65"></td>
								<tr>
								<tr>
							...
							<br>
							<table class="curvedEdges" width="620" border="1" cellpadding="0" cellspacing="0" bordercolor="#FF6633">
								<td colspan=9 bgcolor="#FF6633"><div align="center"><strong><font color="#FFFFFF">Masterlist of Grade Level</font></strong></div></td>
								<tr>
									<th>Grade Level</th>
									<th>Section</th>
									<th>Capacity</th>
								...
								<?php require_once('myFunctions.php'); displayGradeLevelSectionMasterlist(); ?>
							</table>							
							<br>
							<br>
							<?php require_once('myFunctions.php'); displayPageGradeLevelSection()?>	
			
						<br><br>
						<a href="tools.php" class="myLinks"><< Go Back</a>	</div>
		  		</div>
				<?php
					}
				?>
