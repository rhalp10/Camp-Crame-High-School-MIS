	...
	else
	{
		...
		do {
			...
			$tplidx = $pdf->ImportPage($i); 
			 ...
			 $result =mysql_query("SELECT glm.description FROM gradelevelmaster as glm WHERE glm.gradeid=$gradeid");
			while ($row=mysql_fetch_array($result))
			{
			...
			$result = mysql_query("SELECT sm.lrn, um.lname, um.fname, um.mname FROM usermaster as um, studentmaster as sm WHERE um.userid=sm.userid and sm.gradeid=$gradeid AND sectionid NOT IN ('0','') order by um.lname asc");
